...
class Game:
    def __init__(self, players=[], roundLimit=100):
    ...
    def export_player_history_for_a_game(self, idx=0, folder=False):
        for player in self.players:
            ...
            if player.current_money - player.money > 0:
                ...
                win = False
                winT = "输"
            player.exporter.export_player(folder=folder, idx=idx)


class Play:
    def __init__(self, playTime=40, roundLimit=100, params={}, ob={}, callback=False):
    ...
    def setPlayerFromParams(self, playTime):
        ...
        player8 = Player('滴水', money=10000, rule=4, ruleObject=ruleDrop)

        player9 = Player("十五档", money=10000, rule=4, ruleObject=ruleFifteen30000)
    ...
    def play_once(self, playTime):
        ...
        stake_total = 0
        for i in range(l):
            players = self.setPlayerFromParams(playTime)

            # player2=Player('随机',money=310000,)

            # players.append(player2)
            G = Game(players, self.roundLimit)
            ...
            j += 1
            profit += differ
            G.export_player_history_for_a_game(idx=playTime, folder=self.params['output_path'])
            burst_profit = 0
            if player.buster:
                burst_profit -= differ
            no_burst_profit = 0
